    ...
    <script language="javascript">
		$(function(){
			$('.deleteIcon').click(function(e){
				if(delVal('employee')){
					location.href='<cfoutput>#application.baseEPath#/?employee/admin/#isNumeric(local.obj.getID())? local.obj.getID(): "0"#/delete</cfoutput>';
				}
			});
		});
	</script>
...
<cfoutput>
	...
	<!--- form --->
    #local.fbObj.startForm(action='#application.baseEPath#/?employee/admin/#isNumeric(local.obj.getID())? local.obj.getID(): "0"#/process',formID='adminForm')#
    ...
    <cfif val(local.obj.getID())>
    	#local.fbObj.endForm(hasCancel=true,cancelURL='#application.baseEPath#/?employee/admin',hasCustom=true,customText='Delete',customClass='deleteIcon deleteButton')#
	<cfelse>
	    #local.fbObj.endForm(hasCancel=true,cancelURL='#application.baseEPath#/?employee/admin')#
    </cfif>
</cfoutput>